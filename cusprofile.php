...
$query="select * from customer where username='$p'";
...
if($total!=0)
{
	...
	<div class="lable">
			...
			<button id="submit" type="submit" name="update" value="update" class="su"/><b>submit</b></button>
	</div> 
</div>
		</form>
	...
	if (mysqli_connect_error())
		...
		else
		{
			if(isset($_POST['update']))
			{
			    ...
			    if($count>0)
			    {
					$UPDATE = "UPDATE `customer` SET `name`='$name',`address`='$address',`phno`='$phno',`email`='$email',`passwd`='$passwd' WHERE `username`='$username'";
