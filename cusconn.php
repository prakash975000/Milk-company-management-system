...
if(!empty($username) || !empty($name) || !empty($address) || !empty($phno) || !empty($email) || !empty($passwd) || !empty($cpasswd))
{
	...
	if (mysqli_connect_error())
		...
		else
		{
			...
			$INSERT = "INSERT into customer (username,name,address,phno,email,passwd,cpasswd) values(?,?,?,?,?,?,?)";
			...
			if ($rnum==0)
			{
				...
				$stmt->bind_param("sssssss",$username,$name,$address,$phno,$email,$passwd,$cpasswd);
				$stmt->execute();
